...
public class BaseFormController implements ServletContextAware {
	...
	public  String saveFile(MultipartFile file, String savePath) throws Exception {
		...
		if (!file.isEmpty()) {
			...
			if(".mp3".equalsIgnoreCase(prefix)){
	...
	public  FileInfo saveMP3File(MultipartFile file, String savePath) throws Exception {
		FileInfo fi = new FileInfo();
		if (!file.isEmpty()) {
			createDir(rootPath + savePath);
			String fileName = file.getOriginalFilename();
			fi.setRealFileName(fileName);
			String prefix = fileName.substring(fileName.lastIndexOf("."));
			fi.setPrefix(prefix);
			String s = String.valueOf(	System.currentTimeMillis());
			String newname =  s + prefix;
			String filePath = rootPath + savePath  + "/" + newname;
